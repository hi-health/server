...
class RepeatMultiDirectionAIv1_ManualTune extends AI{



	public function __construct($template, $test, $param, $hyperparams)
    {
    ...
    public function calScore()
    {
        ...
        for ($i=0; $i < $this->session; $i++) { 
            ...
            for ($j=0; $j < $this->test_repeat_time; $j++) { 
                ...
                $score_1session['acc_x'] += max( $this->hyperparams['score_x'] * 20/3 * $isGoodSpeed[$i][$j]['acc_x'][0] / $this->test_repeat_time, 0 );
                $score_1session['acc_y'] += max( $this->hyperparams['score_y'] * 20/3 * $isGoodSpeed[$i][$j]['acc_y'][0] / $this->test_repeat_time, 0 );
                $score_1session['acc_z'] += max( $this->hyperparams['score_z'] * 20/3 * $isGoodSpeed[$i][$j]['acc_z'][0] / $this->test_repeat_time, 0 );

                $score_1session['goodMove_roll'] += $this->hyperparams['score_roll'] * 80 * $isGoodMove[$i][$j]['roll'][0]/count($isGoodMove[$i][$j])/$this->test_repeat_time;
                $score_1session['goodMove_yaw'] += $this->hyperparams['score_yaw'] * 80 * $isGoodMove[$i][$j]['yaw'][0]/count($isGoodMove[$i][$j])/$this->test_repeat_time;
                $score_1session['goodMove_pitch'] += $this->hyperparams['score_pitch'] * 80 * $isGoodMove[$i][$j]['pitch'][0]/count($isGoodMove[$i][$j])/$this->test_repeat_time;

                $acc_x_var_ratio += $isGoodSpeed[$i][$j]['acc_x'][0];
                $acc_y_var_ratio += $isGoodSpeed[$i][$j]['acc_y'][0];
                $acc_z_var_ratio += $isGoodSpeed[$i][$j]['acc_z'][0];

                $roll_amp_ratio += $isGoodMove[$i][$j]["roll"][0];
                $yaw_amp_ratio += $isGoodMove[$i][$j]["yaw"][0];
                $pitch_amp_ratio += $isGoodMove[$i][$j]["pitch"][0];

                $acc_x_var_sign += $isGoodSpeed[$i][$j]['acc_x'][1];
                $acc_y_var_sign += $isGoodSpeed[$i][$j]['acc_y'][1];
                $acc_z_var_sign += $isGoodSpeed[$i][$j]['acc_z'][1];

                $roll_amp_sign += $isGoodMove[$i][$j]["roll"][1];
                $yaw_amp_sign += $isGoodMove[$i][$j]["yaw"][1];
                $pitch_amp_sign += $isGoodMove[$i][$j]["pitch"][1];
            }

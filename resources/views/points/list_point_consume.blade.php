...
<header class="w3-panel w3-center w3-opacity" style="padding:128px 16px">
  ...
  <h1>Your Remained Points : {{$RemainedPoint}} </h1>
  ...
  <div class="w3-padding-32">
    ...
    <table align="center" style="width:30%">
      ...
      @foreach($PointConsume as $PointConsumes)
      <tbody>
        <tr>
          <td>{{ $PointConsumes->point }}</td>
          <td>{{ $PointConsumes->transaction->users_id }}</td>
          <td>{{ $PointConsumes->created_at->format('Y-m-d') }}</td>
        </tr>
      </tbody>
      @endforeach
    </table>

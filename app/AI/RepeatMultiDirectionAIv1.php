...
class RepeatMultiDirectionAIv1 extends AI{
    ...
    protected function calFeature()
    {
        ...
        foreach (['acc_x', 'acc_y', 'acc_z', 'roll', 'yaw', 'pitch'] as $axis) {
    ...
    protected function validateMove()
    {
        ...
        foreach (['roll', 'yaw', 'pitch'] as $axis) {
            for ($i=0; $i < $this->session; $i++) { 
                for ($j=0; $j < $this->test_repeat_time; $j++) { 
                    for ($k=0; $k < count($this->templateData[$axis]); $k++) { 
                        $t = $feature["test"][$i][$axis]['amp'][$j];
                        $T = $feature["template"][$axis]['amp'][$k];
                        if($t<0.2 && $T<0.2){//both < 10deg
                            $isGoodMove[$i][$j][$axis] += 1 / count($this->templateData[$axis]);
                        }
                        else{
                            $isGoodMove[$i][$j][$axis] += min($t/$T,$T/$t) / count($this->templateData[$axis]);
    ...
    protected function validateSpeed()
    {
        ...
        foreach (['acc_x', 'acc_y', 'acc_z'] as $axis) {
            for ($i=0; $i < $this->session; $i++) { 
                for ($j=0; $j < $this->test_repeat_time; $j++) { 
                    for ($k=0; $k < count($this->templateData[$axis]); $k++) { 
                        $t = $feature["test"][$i][$axis]['var'][$j];
                        $T = $feature["template"][$axis]['var'][$k];
                        $isGoodSpeed[$i][$j][$axis] += min($t/$T,$T/$t) / count($this->templateData[$axis]);
                            //( 1 - abs($t / $T - 1) ) / count($this->templateData[$axis]);
                    }
                }
            }
        ...
        for ($i=0; $i < $this->session; $i++) { 
            ...
            if($this->testTime[$i]/$this->test_repeat_time < $this->templateTime/5/3){
                ...
                for ($j=0; $j < $this->test_repeat_time; $j++) { 
                    ...
                    $score_1session['acc_x'] += max( 20/3 * $isGoodSpeed[$i][$j]['acc_x'] / $this->test_repeat_time, 0 );
                    $score_1session['acc_y'] += max( 20/3 * $isGoodSpeed[$i][$j]['acc_y'] / $this->test_repeat_time, 0 );
                    $score_1session['acc_z'] += max( 20/3 * $isGoodSpeed[$i][$j]['acc_z'] / $this->test_repeat_time, 0 );

                    $score_1session['goodMove_roll'] += 80 * $isGoodMove[$i][$j]['roll']/count($isGoodMove[$i][$j])/$this->test_repeat_time;
                    $score_1session['goodMove_yaw'] += 80 * $isGoodMove[$i][$j]['yaw']/count($isGoodMove[$i][$j])/$this->test_repeat_time;
                    $score_1session['goodMove_pitch'] += 80 * $isGoodMove[$i][$j]['pitch']/count($isGoodMove[$i][$j])/$this->test_repeat_time;

                    $acc_x_var_ratio += $isGoodSpeed[$i][$j]['acc_x'];
                    $acc_y_var_ratio += $isGoodSpeed[$i][$j]['acc_y'];
                    $acc_z_var_ratio += $isGoodSpeed[$i][$j]['acc_z'];

                    $roll_amp_ratio += $isGoodMove[$i][$j]["roll"];
                    $yaw_amp_ratio += $isGoodMove[$i][$j]["yaw"];
                    $pitch_amp_ratio += $isGoodMove[$i][$j]["pitch"];
                }
                ...
                $arr_gyro = [   array_sum($feature["test"][$i]['roll']['amp']),
                                array_sum($feature["test"][$i]['yaw']['amp']),
                                array_sum($feature["test"][$i]['pitch']['amp'])
                            ];
